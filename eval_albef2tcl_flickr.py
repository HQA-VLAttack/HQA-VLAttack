...
from models.model_retrieval_HQA import ALBEF
from models.vit_HQA import interpolate_pos_embed
...
def retrieval_eval(model, ref_model, t_model, t_ref_model, t_test_transform, data_loader, tokenizer, t_tokenizer, device, config):
    ...
    for batch_idx, (images, texts_group, images_ids, text_ids_groups) in enumerate(data_loader):
        ...
        for i in range(adv_images.shape[0]):
            tem_json = {}
            adv_txt_indices = [index for index, value in enumerate(txt2img) if value == i]
            tem_json['caption'] = []
            for i1 in adv_txt_indices:
                tem_json['caption'].append(adv_texts[i1])
            json_file.append(tem_json)
        ...
        with torch.no_grad():
            ...
            s_output_img = model.Gen_image_feats(s_adv_images_norm)
            s_output_txt = model.Gen_text_feats(adv_texts_input)
            #image_feat是encoder后的cls嵌入经过投影后的维度低一点特征，image_embed是encoder后的特征，含有多个分片
            ...
            adv_texts_input = tokenizer(adv_texts, padding='max_length', truncation=True, max_length=30, 
                                        return_tensors="pt").to(device)            
            t_output_img = t_model.Gen_image_feats(t_adv_images_norm)
            t_output_txt = t_model.Gen_text_feats(adv_texts_input)
    ...
    with open(f'captions_flickr_{args.source_model}.json', 'w', encoding='utf-8') as f:
        json.dump(json_file,f, ensure_ascii=False, indent=4)
    return s_score_matrix_i2t.cpu().numpy(), s_score_matrix_t2i.cpu().numpy(), t_score_matrix_i2t.cpu().numpy(), t_score_matrix_t2i.cpu().numpy()#\
...
def retrieval_score(model, image_feats, image_embeds, text_feats, text_embeds, text_atts, num_image, num_text, device=None):
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix, 50, header)):
        ...
        #=================================有改动=============================
        output,_ = model.text_encoder(encoder_embeds=text_embeds[topk_idx].to(device),
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix, 50, header)):
        ...
        encoder_att = torch.ones(encoder_output.size()[:-1], dtype=torch.long).to(device)
        #=================================有改动=============================
        output,_ = model.text_encoder(encoder_embeds=text_embeds[i].repeat(config['k_test'], 1, 1).to(device),
...
def itm_eval(scores_i2t, scores_t2i, img2txt, txt2img, model_name):
    ...
    origin_tr1 = np.load(f'{original_rank_index_path}/{model_name}_tr1_rank_index.npy')#这个代表召回率1成功的文本下标
    ...
    # #==============================================保存结果=====================================
    # # 将数组保存为 .npy 文件
    # np.save(f'/data/lijiaqi/MultimodalAttack/SGA/filck_eval_test/scores_i2t_{model_name}.npy', scores_i2t)
    # np.save(f'/data/lijiaqi/MultimodalAttack/SGA/filck_eval_test/scores_t2i_{model_name}.npy', scores_t2i)

    # np.save(f'/data/lijiaqi/MultimodalAttack/SGA/filck_eval_test/after_attack_tr1_{model_name}.npy', after_attack_tr1)
    # np.save(f'/data/lijiaqi/MultimodalAttack/SGA/filck_eval_test/after_attack_tr5_{model_name}.npy', after_attack_tr5)
    # np.save(f'/data/lijiaqi/MultimodalAttack/SGA/filck_eval_test/after_attack_tr10_{model_name}.npy', after_attack_tr10)

    # np.save(f'/data/lijiaqi/MultimodalAttack/SGA/filck_eval_test/after_attack_ir1_{model_name}.npy', after_attack_ir1)
    # np.save(f'/data/lijiaqi/MultimodalAttack/SGA/filck_eval_test/after_attack_ir5_{model_name}.npy', after_attack_ir5)
    # np.save(f'/data/lijiaqi/MultimodalAttack/SGA/filck_eval_test/after_attack_ir10_{model_name}.npy', after_attack_ir10)
    #=========================================================================
...
def eval_asr(model, ref_model, tokenizer, t_model, t_ref_model, t_tokenizer, t_test_transform, data_loader, device, args, config):
    ...
    if args.source_model == 'RN101':
        s_name = 'CLIP_CNN'
    elif args.source_model in ['ALBEF','ALBEF_pre']:
        s_name = 'ALBEF'
    ...
    if args.target_model == 'RN101':
        t_name = 'CLIP_CNN'
    elif args.target_model == 'ALBEF':
        t_name = 'ALBEF'
    elif args.target_model == 'TCL':
        t_name = 'TCL'
    elif args.target_model == 'ViT-B/16':
        t_name = 'CLIP_ViT'
    t_result = itm_eval(t_score_i2t, t_score_t2i, data_loader.dataset.img2txt, data_loader.dataset.txt2img, t_name)
    #print('Performance on {}: \n {}'.format(args.target_model, t_result))

    result = itm_eval(score_i2t, score_t2i, data_loader.dataset.img2txt, data_loader.dataset.txt2img, s_name)
