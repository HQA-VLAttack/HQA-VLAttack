...
from models.vit_HQA import VisionTransformer
from models.xbert_HQA import BertConfig, BertModel
...
class ALBEF(nn.Module):
    ...
    def Gen_image_feats(self, image):
        image_embed, img_feats_list = self.visual_encoder(image)
        image_feat = F.normalize(self.vision_proj(image_embed[:, 0, :]), dim=-1)
        return {'image_feat': image_feat, 'image_embed': image_embed, 'img_feats_list':img_feats_list}

    def Gen_text_feats(self, text_input):
        text_output, text_feats_list = self.text_encoder(text_input.input_ids, attention_mask=text_input.attention_mask,
                                        mode='text')

        text_embed = text_output.last_hidden_state
        text_feat = F.normalize(self.text_proj(text_embed[:, 0, :]), dim=-1)
        return {'text_feat': text_feat, 'text_embed': text_embed, 'text_feats_list':text_feats_list}

    def interface_skip_image(self, skip_layer, image):
        image_embed= self.visual_encoder.forward_skip(image, skip_layer)
        image_feat = F.normalize(self.vision_proj(image_embed[:, 0, :]), dim=-1)
        return {'image_feat': image_feat, 'image_embed': image_embed}
    
    def interface_skip_text(self, skip_layer, text_input):
        text_output = self.text_encoder.forward_skip(text_input.input_ids, attention_mask=text_input.attention_mask,
                                        mode='text',skip_layer=skip_layer)
