...
class paired_dataset(Dataset):
    def __init__(self, ann_file, transform, image_root, max_words=30):
        ...
        self.txt2img = {}
        self.img2txt = {}
        #self.tar_ann = json.load(open(tar_ann_file, 'r'))
    ...
    def __getitem__(self, index):
        ...
        # tar_texts = self.tar_ann[index]['caption']
...
class target_dataset(Dataset):
    ...
    def __getitem__(self, index):
        texts = self.ann[index]['caption']#[self.text[i] for i in self.img2txt[index]]
        return texts, index

    def collate_fn(self, batch):
        txt_groups,  text_ids_groups = list(zip(*batch))        
        return list(txt_groups), text_ids_groups